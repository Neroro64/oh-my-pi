...
function toSourceFilterLevel(value: unknown, fallback: RtkSourceFilterLevel): RtkSourceFilterLevel {
	return RTK_SOURCE_FILTER_LEVELS.includes(value as RtkSourceFilterLevel) ? (value as RtkSourceFilterLevel) : fallback;
}
...
export function normalizeRtkIntegrationConfig(raw: unknown): RtkIntegrationConfig {
	...
	const hasReadCompaction = hasOwn(outputCompactionSource, "readCompaction");
	...
	return {
		...
		guardWhenRtkMissing: toBoolean(source.guardWhenRtkMissing, DEFAULT_RTK_INTEGRATION_CONFIG.guardWhenRtkMissing),
		showRewriteNotifications: toBoolean(
			source.showRewriteNotifications,
			DEFAULT_RTK_INTEGRATION_CONFIG.showRewriteNotifications,
		),
		outputCompaction: {
			enabled: toBoolean(outputCompactionSource.enabled, DEFAULT_RTK_INTEGRATION_CONFIG.outputCompaction.enabled),
			stripAnsi: toBoolean(
...
export function saveRtkIntegrationConfig(config: RtkIntegrationConfig, configPath = CONFIG_PATH): ConfigSaveResult {
