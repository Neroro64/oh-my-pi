...
import { isToolCallEventType } from "../../../extensibility/extensions/types";
...
export default function rtkIntegrationExtension(pi: ExtensionAPI): void {
	...
	pi.on("before_agent_start", async (event, ctx) => {
		...
		return {
			systemPrompt: [...event.systemPrompt, SOURCE_FILTER_TROUBLESHOOTING_NOTE],
		};
	});
	...
	pi.on("tool_result", async (event, ctx) => {
		...
		try {
			...
			return {
				content: outcome.content as (TextContent | ImageContent)[],
