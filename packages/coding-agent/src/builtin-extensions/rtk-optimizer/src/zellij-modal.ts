...
import { type ExtensionAPI, getSettingsListTheme, type Theme } from "@oh-my-pi/pi-coding-agent";
import {
	Box,
	Container,
	Ellipsis,
...
export class ZellijModalFrame {
	...
	renderContentLine(content: string, width: number, palette: ZellijColorPalette): string {
		...
		const paddedContent = truncateToWidth(content, innerWidth, Ellipsis.Omit, true);
	...
	private renderBottomLine(width: number, palette: ZellijColorPalette): string {
		...
		const safeHelp = truncateToWidth(helpText, Math.max(0, innerWidth - 3), Ellipsis.Unicode);
	...
	private resolveTitleSegment(
		...
		if (typeof segment === "string") {
			const color: keyof ZellijColorPalette =
				position === "left" ? "accent" : position === "center" ? "muted" : "dim";
			return {
				text: ` ${segment} `,
				color,
				bold: position === "left",
	...
	private fitTextToWidth(text: string, maxWidth: number, mode: TitleSegment["truncate"]): string {
		...
		switch (mode) {
			case "none":
				return truncateToWidth(text, maxWidth, Ellipsis.Omit);
			case "start":
				return truncateStart(text, maxWidth);
			case "middle":
				return truncateMiddle(text, maxWidth);
			default:
				return truncateToWidth(text, maxWidth, Ellipsis.Unicode);
		}
	}
	...
	private resolveHelpText(maxWidth: number): string | null {
		...
		if (config.keyHints && config.keyHints.length > 0) {
			...
			return config.keyHints.map(hint => `${hint.key} ${hint.description}`).join(separator);
		}
	...
	render(width: number): string[] {
		...
		try {
			...
			for (const line of normalized) {
				const fitted = truncateToWidth(line, contentWidth, Ellipsis.Omit, true);
			...
			const safe = truncateToWidth(` Render error: ${message} `, paddedWidth, Ellipsis.Unicode, true);
			lines.push(safe);
		}
...
export class ZellijSettingsModal implements ZellijModalContentRenderer {
	...
	render(width: number): string[] {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			return [
				this.theme.fg("error", truncateToWidth(`Settings render error: ${message}`, safeWidth, Ellipsis.Unicode)),
			];
		}
	}
...
function truncateStart(text: string, maxWidth: number): string {
	...
	return `…${truncateToWidth(current, Math.max(0, maxWidth - 1), Ellipsis.Omit)}`;
}
...
function truncateMiddle(text: string, maxWidth: number): string {
	...
	const head = truncateToWidth(text, headTarget, Ellipsis.Omit);
